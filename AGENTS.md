...
이 문서는 `AIStudioMCP` 저장소에서 작업하는 AI 에이전트의 시작 문서입니다.

## 프로젝트 개요

- 주제: 사용자가 영상 주제를 말하면 장면 스크립트 생성, 이미지 생성, 영상 생성까지 이어지는 결과물을 만들어주는 MCP 서버
- 유형: 바이브 코딩 기반 MCP(Model Context Protocol) 서버 제작
- 주요 환경: Node.js, JavaScript, `@modelcontextprotocol/sdk`
- 프로젝트명: `AIStudioMCP`
- 방식: 스크립트 생성 → 이미지 생성 → 영상 생성 순서로 tool 하나씩 동작 검증한 뒤 다음 tool을 추가하는 방식으로 단계별로 구현합니다.
...
- `docs/STYLEGUIDE.md`: JavaScript/MCP tool 작성 규칙
...
- 구조 변경 또는 새 폴더/tool/resource 추가 전 → `docs/ARCHITECTURE.md`
- 코드 수정 또는 새 tool 구현 전 → `docs/STYLEGUIDE.md`
